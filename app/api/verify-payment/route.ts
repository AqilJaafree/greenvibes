...
export async function POST(req: NextRequest) {
  ...
  for (const ix of instructions) {
    ...
    const receiverMatches =
      info.destination === (await getReceiverAta());
    const amountMatches =
      BigInt(info.tokenAmount?.amount ?? "0") === USDC_TRANSFER_AMOUNT;
    const senderMatches =
      info.authority === performerWallet;
...
// Compute the receiver's ATA address on demand (cached after first call)
